...
CMD ["sh", "-c", "python manage.py migrate && mysql --protocol=TCP -h $MYSQLHOST -u $MYSQLUSER -p$MYSQLPASSWORD $MYSQL_DATABASE < /app/init-db.sql && python manage.py runserver 0.0.0.0:8000"]
